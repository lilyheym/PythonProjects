...
def onDelete(self):
      ...
      with conn:
            ...
            count = cur.fetchone()[0]
            if count > 1:
                  confirm = messagebox.askokcancel("Delete Confirmation", "All information assosciated with, ({}) \nwill be permenantly deleted from the database. \n\nProceed with the deletion request?".format(var_select)
                  if confirm:
...
def onDeleted(self):
      ...
      self.txt_phone.delete(0,END)
      self.txt_email.delete(0,END)
##      onRefresh(self)  # update the listbox of thechanges
      try:
...
def onRefresh(self):
      ...
      with conn:
            ...
            i = 0
            while i < count:
                  cursor.execute("""SELEVT col_fullname FROM tbl_phonebook""")
...
def onUpdate(self):
      try:
            ...
            messagebox.showinfo("Missing selection", "No name was selected from the list box. \nCancaling the Update request.")
            return
      # The user will only be allowed to update changes for phone and emails
      # For name cahnges, the user will need to delete the entire record and start over
      var_phone = self.txt_phone.get().strip() # normalize the data to amintain database integrity
      var_email = self.txt_email.get().strip()
      if (len(var_phone) > 0) and (len(var_email) > 0): # ensure that there is data present
            conn = sqlite3.connect('phonebook.db')
            with conn:
                  cur = conn.cursor()
                  # count records to see if the user's changes are aleady in
                  # the databases...meaining there are no changes tp update.
                  ...
                  if count == 0 or count2 == 0: # if proposed changes are not already in the database, then proceed
                        ...
                        if response:
                              with conn:
                                    cursor = conn.cursor()
                                    cursor.execute("""UPDATE tbl_phonnebook SET col_phone = '{0}', col_email = '{1}' WHERE col_fullname = '{2}'""".format(var_phone,var_email,var_fullname))
                                    onClear(self)
                                    conn.commit()
                        else:
                              messagebox.showinfo("Cancel request","No changes have been made to ({}).".format(var_value))
                  else:
                        messagebox.showinfo("No changes detected","Both ({}) and ({}) \nalready exist in the database for this name. \n\nYour update request has been denied".format(var_phone,var_email))
                  onClear(self)
            conn.close()
      else:
